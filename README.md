...
- **Forward (default)** — systemd-resolved forwards to the selected resolvers. **Providers:** Cloudflare, Google, Quad9, OpenDNS (the non-filtering Sandbox pair `208.67.222.2`/`208.67.220.2`, DoT `sandbox.opendns.com`), AdGuard (the non-filtering pair `94.140.14.140`/`94.140.14.141`, DoT `unfiltered.adguard-dns.com`), Custom. Alibaba (223.5.5.5/223.6.6.6, DoT `dns.alidns.com`) and DNSPod (119.29.29.29/119.28.28.28, DoT `dot.pub`) are China-optimized anycast and are offered only on mainland-China networks: setup resolves `google.com` with a direct-IP query (Alibaba, fallback Cloudflare — the local resolver's speed never enters the picture) and then TCP-connects to the returned address; the name resolving but the connection failing within 3 seconds means mainland China, so both are added, because their POPs win the latency race there and lose it everywhere else. If neither resolver answers, the region is undeterminable and the gate fails closed (no China providers). Filtering resolver flavours (AdGuard Default, OpenDNS Standard) are deliberately not used — servers run unattended ACME DNS-01 and background jobs — and `Custom DNS` covers anything else
...
`clikader dns list` (or `ls`) prints the current resolvers without touching
anything: the managed configuration (clikader drop-in or `resolved.conf`),
the live `resolvectl dns` state, and the `/etc/resolv.conf` target. Read-only,
so it works as a normal user (no sudo needed).
